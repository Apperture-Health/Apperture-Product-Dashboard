...
def _session_payload(username: str) -> AuthSessionDTO:
    ...
    return AuthSessionDTO(
        ...
        allowed_indications=access.get("disease_areas"),
        allowed_atc_classes=access.get("drug_classes"),
    )
...
def pipeline_page(payload: PageRequest, request: Request) -> dict:
    _require_auth(request)
    filters = _filter_state(payload.filters, request)
    indication = filters.indication_name
    sponsors = tuple(filters.sponsor)
    trials_df = get_pipeline_trials_table(indication)
    if filters.sponsor and not trials_df.empty:
        trials_df = trials_df[trials_df["sponsor_name"].isin(filters.sponsor)]
    heat_df = get_pipeline_sponsor_indication_heatmap(indication, sponsors)
    return {
        "kpis": get_pipeline_kpis(indication, sponsors),
        "bySponsor": _records(get_pipeline_by_sponsor(indication, sponsors, limit=20)),
        "byIndication": _records(get_pipeline_by_indication(indication, sponsors, limit=25)),
        "topInterventions": _records(get_pipeline_top_interventions(indication, sponsors, limit=25)),
        "sponsorIndicationHeatmap": _records(heat_df),
        "proUsage": _records(get_pipeline_pro_usage(indication, sponsors, limit=20)),
...
def outcome_scores_page(payload: PageRequest, request: Request) -> dict:
    ...
    return {
        ...
        "message": "This page is currently a work in progress.",
    }
...
def ai_page_summary(page_key: str, payload: AiPageSummaryRequest, request: Request) -> dict[str, str]:
    ...
    if page_key == "drug-detail":
        ...
        sponsors = tuple(filters.sponsor)
        kpis = get_pipeline_kpis(indication, sponsors)
        sp_df = get_pipeline_by_sponsor(indication, sponsors, limit=20)
        ind_df = get_pipeline_by_indication(indication, sponsors, limit=25)
        intv_df = get_pipeline_top_interventions(indication, sponsors, limit=25)
        pro_df = get_pipeline_pro_usage(indication, sponsors, limit=20)
        trials_df = get_pipeline_trials_table(indication)
        if filters.sponsor and not trials_df.empty:
            trials_df = trials_df[trials_df["sponsor_name"].isin(filters.sponsor)]
